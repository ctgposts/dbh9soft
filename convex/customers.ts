...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    // ✅ UNLIMITED: Support unlimited customers - no hard limit
    // Use args.limit for pagination if provided, otherwise return all
    ...
    // Apply pagination AFTER filtering (if limit provided)
    if (args.limit && args.limit > 0) {
      const offset = args.offset || 0;
      customers = customers.slice(offset, offset + args.limit);
    ...
    // Return all customers if no limit specified
  },
});
