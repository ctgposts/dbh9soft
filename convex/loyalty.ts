...
export const getPointsTransactions = query({
  ...
  handler: async (ctx, args) => {
    // ✅ UNLIMITED: Load all points transactions without limit
    return await ctx.db
...
export const getTopCustomers = query({
  ...
  handler: async (ctx, args) => {
    // ✅ UNLIMITED: Load all customers and sort by points
    const customers = await ctx.db.query("customerLoyalty").collect();
    const topCustomers = customers.sort((a, b) => b.totalPoints - a.totalPoints);
    
    // Apply limit if provided, otherwise return all
    if (args.limit && args.limit > 0) {
      return topCustomers.slice(0, args.limit);
    }
    return topCustomers;
  },
});
