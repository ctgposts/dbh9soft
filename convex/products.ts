...
/**
 * Migration: Ensure all products have unique barcodes
 * ✅ বসা প্রোডাক্টের জন্য বারকোড জেনারেট করুন
 * ✅ ডিউপ্লিকেট বারকোড রিসলভ করুন
 */
export const ensureAllProductsHaveBarcode = mutation({
  args: {},
  handler: async (ctx) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Not authenticated");
    ...
    const barcodeMap = new Map<string, number>(); // Track barcode usage

    // First pass: collect all existing barcodes
    for (const product of allProducts) {
    ...
    // Second pass: generate or fix barcodes
    for (const product of allProducts) {
      // Skip if product already has a valid unique barcode
      if (
        product.barcode &&
        product.barcode.trim().length > 0 &&
        barcodeMap.get(product.barcode) === 1
      ) {
        skippedCount++;
        continue;
      }
      ...
      do {
        // ফরম্যাট: {ProductCode}{Price}
        const productCode = product.productCode || `AB${product._id.substring(0, 6).toUpperCase()}`;
        const priceDigits = Math.round(product.sellingPrice * 100)
          .toString()
          .padStart(4, '0');
        
        // Add attempt number if collision detected
        const suffix = attempts > 0 ? attempts.toString() : '';
        newBarcode = `${productCode}${priceDigits}${suffix}`;

        // Check if this barcode already exists in database
        const existingProduct = await ctx.db
          .query("products")
          .filter((q) => q.eq(q.field("barcode"), newBarcode))
          .first();
      ...
      if (!isUnique) {
        throw new Error(
          `Failed to generate unique barcode for product ${product.name} after ${maxAttempts} attempts`
        );
      }
    ...
    return {
      ...
      skippedCount,
      summary: {
        newBarcodes: generatedCount,
        duplicatesFixed: duplicateResolvedCount,
        alreadyHadUnique: skippedCount,
      },
    };
  },
});
...
/**
 * Query: Get barcode audit report
 * ✅ সব প্রোডাক্ট এবং তাদের বারকোড স্ট্যাটাস দেখুন
 */
export const getBarcodeAuditReport = query({
  args: {},
  handler: async (ctx) => {
    const allProducts = await ctx.db.query("products").collect();
    ...
    const report = {
      ...
      withBarcode: 0,
      withoutBarcode: 0,
      duplicateBarcodes: [] as { barcode: string; products: string[] }[],
      productsWithoutBarcode: [] as {
        _id: string;
        name: string;
        brand: string;
        price: number;
    ...
    const barcodeMap = new Map<string, string[]>(); // barcode -> product IDs

    for (const product of allProducts) {
      if (product.barcode && product.barcode.trim().length > 0) {
        report.withBarcode++;
        const ids = barcodeMap.get(product.barcode) || [];
        ids.push(product._id);
        barcodeMap.set(product.barcode, ids);
      } else {
        report.withoutBarcode++;
        report.productsWithoutBarcode.push({
          _id: product._id,
          name: product.name,
          brand: product.brand,
    ...
    // Find duplicates
    for (const [barcode, productIds] of barcodeMap.entries()) {
